...
bool is_critical(invalid_argument const &er) { return true; }

BOOST_AUTO_TEST_CASE(initialization_check) {
    BOOST_TEST(l1.begin() == nullptr);
    BOOST_TEST(l1.end() == nullptr);
...
BOOST_AUTO_TEST_CASE(insert_and_to_string_check) {
    l1.insert(0, 5);
    BOOST_TEST(l1.to_string().compare("5\n") == 0);
    BOOST_CHECK_EXCEPTION(l1.insert(5, 100), invalid_argument, is_critical);
    l1.insert(1, 100);
    BOOST_TEST(l1.to_string().compare("5, 100\n") == 0);
    l1.insert(0, 10);
    string p = l1.to_string();
...
BOOST_AUTO_TEST_CASE(remove_check)  {
    l1.fill(1, 5);
    BOOST_CHECK_EXCEPTION(l1.remove(100), invalid_argument, is_critical);
    l1.remove(5);
